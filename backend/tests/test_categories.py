def test_categories_crud_and_conflict(client):
    ...
    res = client.post("/api/categories", json={"name": cat_name})
    ...
    res_dup = client.post("/api/categories", json={"name": cat_name.lower()})
    assert res_dup.status_code == 409
    assert "already exists" in res_dup.json()["detail"]

    # 4. Update category
    updated_name = "TestCategory_Renamed1"
    res_up = client.put(f"/api/categories/{cat_id}", json={"name": updated_name})
...
def test_delete_category_blocked_when_expenses_linked(client):
    # Create category and expense linked to it
    cat_res = client.post("/api/categories", json={"name": "LinkedCat_Test"})
