def test_category_validation_edge_cases(client):
    # Empty / whitespace category name
    res1 = client.post("/api/categories", json={"name": "   "})
    assert res1.status_code == 422

    # Category name exceeding 100 characters
    res2 = client.post("/api/categories", json={"name": "A" * 101})
    assert res2.status_code == 422

    # GET non-existent category
    res3 = client.get("/api/categories/999999")
    assert res3.status_code == 404

    # PUT non-existent category
    res4 = client.put("/api/categories/999999", json={"name": "NonExistent"})
    assert res4.status_code == 404

    # DELETE non-existent category
    res5 = client.delete("/api/categories/999999")
    assert res5.status_code == 404

def test_expense_validation_edge_cases(client):
    # Fetch valid category
    cat_res = client.get("/api/categories")
    cat_id = cat_res.json()[0]["id"]

    # Amount = 0
    res1 = client.post("/api/expenses", json={
        "amount": 0,
        "category_id": cat_id,
    ...
    res4 = client.get("/api/expenses/999999")
    assert res4.status_code == 404

    # PUT non-existent expense
    res5 = client.put("/api/expenses/999999", json={
        "amount": 100,
        "category_id": cat_id,
        "expense_date": "2026-08-26"
    })
    ...
    res6 = client.delete("/api/expenses/999999")
