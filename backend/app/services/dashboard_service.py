...
def get_dashboard_summary(db: Session, user_id: int) -> DashboardSummaryResponse:
    # SQL Aggregation for total expense and count for current user
    ...
    # 5 most recent expenses for current user
    ...
    # Category summary SQL aggregation for current user
    category_summary_rows = db.query(
     ...
     .filter(Category.user_id == user_id, Expense.user_id == user_id)\
    ...
    # Payment Mode summary SQL aggregation for current user
    ...
    daily_budget_progress = get_daily_budget_progress(db, user_id)
